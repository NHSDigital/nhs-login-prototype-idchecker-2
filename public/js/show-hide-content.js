...
(function (global) {
  ...
  function ShowHideContent() {
    var self = this; // Radio and Checkbox selectors
    ...
    function initToggledContent() {
      var $control = $(this);
      var $content = getToggledContent($control); // Set aria-controls and defaults
    ...
    function getToggledContent($control) {
      var id = $control.attr('aria-controls'); // ARIA attributes aren't set before init
    ...
    function showToggledContent($control, $content) {
      ...
      if ($content.hasClass('js-hidden')) {
        ...
        $content.attr('aria-hidden', 'false'); // If the controlling input, update aria-expanded
    ...
    function hideToggledContent($control, $content) {
      $content = $content || getToggledContent($control); // Hide content

      if (!$content.hasClass('js-hidden')) {
        $content.addClass('js-hidden');
        $content.attr('aria-hidden', 'true'); // If the controlling input, update aria-expanded
    ...
    function handleRadioContent($control, $content) {
      ...
      var $radios = $form.length ? $form.find(selector) : $(selector); // Hide content for radios in group
    ...
    function init($container, elementSelector, eventSelectors, handler) {
      $container = $container || $(document.body); // Handle control clicks
      ...
      $controls.each(initToggledContent); // Handle events
    ...
    function getEventSelectorsForRadioGroups() {
      var radioGroups = []; // Build an array of radio group selectors
...
$(document).ready(function () {// new GOVUK.ShowHideContent();
});